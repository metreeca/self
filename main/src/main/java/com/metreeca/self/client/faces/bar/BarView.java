...
final class BarView extends View {
	...
	private final Info info=Info.create("chart.bar", BarModel.defs)
	...
	public BarView() {
		root("<figure/>").is("placeholder bar-chart", true)
	...
	private Tile detail(final JSObject cell) {
		...
		final TermInfo info=new TermInfo().term(model.item(cell));

		final List<Path> values=model.lens(false).getValues();

		for (int j=0; j < values.size(); j++) {
			info.details(values.get(j), j == cell.integer("j"), model.value(j, cell));
		}
	...
	public BarView model(final BarModel model) {
	...
	private void series(final Path path) {
		if ( model.isValue(path) ) {
			root().async(model.lens(true).insertValue(path)); // preferred compatible role
		} else {
